...
const inputVariants = cva(
  "h-9 w-full rounded-md border py-2 text-sm transition-all disabled:cursor-not-allowed disabled:opacity-50 placeholder:text-muted-foreground",
  {
    variants: {
      variant: {
        default: "border-input bg-background focus:outline focus:outline-primary",
        filled: "border-transparent bg-muted focus:outlinefocus:outline-primary",
...
const InputComponent = React.forwardRef<HTMLInputElement, InputRootProps>((props, ref) => {
  ...
  const inputClassName = cn(
    ...
    error && [
      ...
      !["flushedfilled", "flushed"].includes(variant as string) && "focus:outline focus:outline-red-500",
    ],
  ...
  return (
    <InputContext.Provider value={contextValue}>
      <div className="relative">
        ...
        />
        {maxLength && (
          <div className={cn("absolute top-2.5 text-xs text-muted-foreground", counterPosition)}>
            {String(value || "").length}/{maxLength}
          </div>
        )}
        {error && textError && <p className="mt-1 text-xs text-red-500">{textError}</p>}
      </div>
...
const InputLabel = React.forwardRef<HTMLLabelElement, React.LabelHTMLAttributes<HTMLLabelElement>>((props, ref) => {
  ...
  const { id, isFocused, required, value, disabled, error, variant, elementChecks, type, placeholder } =
    useInputContext();
  ...
  const labelClassName = cn(
    ...
    (isFocused || value || type === "date" || placeholder) && [
      ...
      variant === "flushed" || variant === "flushedfilled" ? "-left-[7px] ps-2" : "left-[7px] border-2",
    ],
    error && "text-red-500",
    disabled && "opacity-50 cursor-not-allowed",
...
const InputLeftIcon = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>((props, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "absolute left-3 top-[8px] flex h-4 w-4 items-center",
        disabled && "opacity-50",
        error && "text-red-500",
        className,
      )}
...
const InputPasswordToggle = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(
  (props, ref) => {
...
const InputClearButton = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(
  (props, ref) => {
