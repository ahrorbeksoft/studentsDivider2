...
export const hasNestedElementOfType = (children: React.ReactNode, types: any[]): boolean => {
  ...
  const checkChild = (child: React.ReactNode): boolean => {
    ...
    if (types.includes(childType) || (childType.displayName && typeDisplayNames.has(childType.displayName))) {
      return true;
    }
